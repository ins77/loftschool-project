...
const findAllPSiblings = (where) => (
    [...where.children].filter(({ nextElementSibling }) => (
...
const collectDOMStat = (root) => {
    const getFlattenNodes = (root) => (
        [...root.childNodes].reduce((acc, currentNode) => {
            const newAcc = currentNode.hasChildNodes() ? [...getFlattenNodes(currentNode), ...acc] : acc;
    
            return [currentNode, ...newAcc];
        }, [])
    );
    
    const getFlattenClasses = (elements) => (
        elements.reduce((acc, currentElement) => (
            [...acc, ...currentElement.classList]
        ), [])
    );
    ...
    // texts
    const textsNodes = flattenNodes.filter(({ nodeType }) => nodeType === 3);
